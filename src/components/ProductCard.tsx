...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Card className="group overflow-hidden transition-all hover:shadow-lg relative flex flex-col h-full rounded-2xl sm:rounded-3xl border-muted/50">
      ...
      <CardContent className="p-3 sm:p-4 flex-grow space-y-1 sm:space-y-1.5">
        <Link href={`/product/${product.id}`} className="block">
          <h3 className="line-clamp-1 font-headline text-xs sm:text-base font-bold group-hover:text-primary transition-colors">
            {product.name}
          </h3>
        </Link>
